...
export default {
  async render(root) {
    ...
    if (!code) {
      mount(root, `<div class="card" style="padding:1rem"><p>Error: Missing OAuth code</p></div>`);
      return;
    }

    try {
      // pozovi backend /callback sa code
      await oauthCallback({ code });

      toast('Gmail account connected!', 'success');
      window.location.href = '/smtp'; // redirect na listu smtp naloga
    } catch (err) {
      console.error(err);
      mount(root, `<div class="card" style="padding:1rem"><p>Error connecting Gmail: ${err.message}</p></div>`);
    }
  }
};
