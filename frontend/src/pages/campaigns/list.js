...
import { openCampaignWizard } from '../../ui/components/CampaignWizardModal.js'; // 👈 ubaci import
...
function view(rows, { q='', page=1 }){
  ...
  <div class="mt-2">
    ${renderTable({
      columns:[
        { header:'Name', accessor:'name', cell:r=>`<a class="campaign-name" href="/campaigns/${r.id}">${r.name}</a>` },
...
const CampaignList = {
  async render(root){
    mount(root, `<div class="card" style="padding:1rem"><div class="row"><div class="spinner"></div><div>Loading campaigns…</div></div></div>`);
    ...
    try{
      ...
      // 🔄 mapiraj backend polja (zameni name i description)
      rows = rows.map(r => ({
        ...r,
        name: r.description,
        description: r.name
...
function wireEvents(root, rows){
  // 👇 interceptiraj klik na Create Campaign
  const createBtn = qs('#createCampaignBtn', root);
  if (createBtn){
    createBtn.addEventListener('click', (e)=>{
      e.preventDefault(); // spreči navigaciju na /create
      openCampaignWizard(); // otvori modal
    });
  }
